...
const profileStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    if (!type || !["CASUAL", "PROFESSIONAL"].includes(type)) {
      return cb(
        new Error("Invalid profile type. Must be CASUAL or PROFESSIONAL.")
      );
    }
...
router.post("/profile", profileUpload.single("file"), async (req, res) => {
  ...
  if (!type || !["CASUAL", "PROFESSIONAL"].includes(type)) {
    return res
      .status(400)
      .json({ error: "Invalid profile type. Must be CASUAL or PROFESSIONAL." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to upload or update profile",
...
router.post(
  "/lesson",
  lessonUpload.array("files", 3),
  verifyToken,
  verifyRoles(["MENTOR"]),
  async (req, res) => {
    const { title, description, deadline, classId, batchId, mentorId } =
      req.body;
    const { files } = req;
    ...
    try {
      ...
      if (!classData || classData.users.length === 0) {
        return res.status(404).json({ error: "No trainees found for this class." });
      }
      ...
      for (const user of classData.users) {
        ...
        const traineeNotification = await prisma.notification.create({
          data: {
            userId: traineeId,
            title: "New Lesson Uploaded",
            description: "A new Lesson has been uploaded for your class.",
            type: "Presentation",
          },
        });

        // Emit socket notification to each trainee
        const io = socket.getIO();
        io.to(traineeId).emit("receiveNotification", {
          id: traineeNotification.id,
          title: traineeNotification.title,
          description: traineeNotification.description,
          type: traineeNotification.type,
          createdAt: traineeNotification.createdAt,
        });

        console.log(`📢 Notification sent to trainee ${traineeId}`);
      }
      ...
      res
        .status(500)
        .json({
          error: "Failed to create lesson and upload files.",
          details: error.message,
        });
      console.log(error);
    }
...
router.put(
  ...
  async (req, res) => {
    ...
    const { title, description, deadline, classId, batchId, deleteOldFiles } =
      req.body;
    const { files } = req;
        ...
        await Promise.all(
          existingLesson.files.map(async (file) => {
            try {
              ...
              console.error(
                `Failed to delete file: ${file.filepath}`,
                err.message
              );
            }
          })
...
const challengeUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "Invalid file type. Only PDF, JPEG, PNG, and ZIP are allowed."
        )
      );
    }
  },
...
router.post(
  "/challenge",
  challengeUpload.array("files", 3),
  verifyToken,
  verifyRoles(["MENTOR"]),
  async (req, res) => {
    const { title, description, batchId, classId, deadline, mentorId } =
      req.body; // Added mentorId
    ...
    if (
      ...
      return res
        ...
        .json({
          error:
            "All fields (title, description, batchId, classId, deadline, mentorId) are required.",
        });
    }
    ...
    if (!files || files.length === 0) {
      return res
        .status(400)
        .json({ error: "File is required for uploading a challenge." });
    }
    ...
    try {
      ...
      for (const user of classData.users) {
        ...
        const traineeNotification = await prisma.notification.create({
          data: {
            userId: traineeId,
            title: "New Presentation Uploaded",
            description: "A new final presentation has been uploaded for your class.",
            type: "Presentation",
          },
        });
  ...
  async (req, res) => {
    try {
      ...
      if (fileName) {
        ...
        return res
          .status(400)
          .json({ error: "Either fileName or coverImage must be provided." });
      }

      const existingCover = await prisma.classCover.findUnique({
        where: { classId },
      });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const existingCover = await prisma.batchCover.findUnique({
        where: { batchId },
      });
...
router.get("/class-cover/:classId", async (req, res) => {
  try {
    ...
    const classCover = await prisma.classCover.findUnique({
      where: { classId },
    });
...
const presentationUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimeTypes.includes(file.mimetype)) {
      ...
      cb(
        new Error(
          "Invalid file type. Only JPEG, PNG, PDF, ZIP, and MP4 are allowed."
        )
      );
    }
  },
...
router.post(
  "/presentation",
  presentationUpload.array("files", 3),
  verifyToken,
  verifyRoles(["MENTOR"]),
  async (req, res) => {
    ...
    if (!title || !description || !deadline || !classId || !batchId) {
      return res.status(400).json({ error: "All presentation fields are required." });
    }
    ...
    try {
      // Fetch all trainees in the class
      const classData = await prisma.class.findUnique({
        where: { id: classId },
        select: {
          users: {
      ...
      // Upload files associated with the presentation
      ...
      // Notify each trainee in the class
      for (const user of classData.users) {
        const traineeId = user.id;
      ...
      res.status(500).json({
        error: "Failed to create presentation and upload files.",
        details: error.message,
      });
    }
  }
